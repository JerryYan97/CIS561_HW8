 ...
 */
    ...
    auto csgCube = std::make_shared<CSGNode>();
    csgCube->mType = GEOMETRY;
    csgCube->selfGeo =longCube.get();
    ...
    csgRoot->mType = DIFFER;
    csgRoot->leftChild = csgSphere.get();
    csgRoot->rightChild = csgCube.get();

    auto csgPrim = std::make_shared<Primitive>(csgRoot);
    csgPrim->material = matteWhite;
    csgPrim->name = QString("csg Primitive");

    CSGShapes.append(longCube);
    CSGShapes.append(csgCube);
