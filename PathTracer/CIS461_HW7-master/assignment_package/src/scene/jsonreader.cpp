    ...
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        foreach(const QJsonValue &frame, frames) {
            ...
            if(sceneObj.contains(QString("camera"))) {
                ...
                // scene.SetThinLensCamera(LoadCamera(camera));
            }
            else if(sceneObj.contains(QString("tlcamera")))
            {
                camera = sceneObj["tlcamera"].toObject();
                // scene.SetCamera(LoadCamera(camera));
                scene.SetThinLensCamera(LoadTLCamera(camera));
            }
...
ThinLensCamera JSONReader::LoadTLCamera(QJsonObject &tlcamera)
{
    ThinLensCamera result;
    if(tlcamera.contains(QString("target"))) result.ref = ToVec3(tlcamera["target"].toArray());
    if(tlcamera.contains(QString("eye"))) result.eye = ToVec3(tlcamera["eye"].toArray());
    if(tlcamera.contains(QString("worldUp"))) result.world_up = ToVec3(tlcamera["worldUp"].toArray());
    if(tlcamera.contains(QString("width"))) result.width = tlcamera["width"].toDouble();
    if(tlcamera.contains(QString("height"))) result.height = tlcamera["height"].toDouble();
    if(tlcamera.contains(QString("fov"))) result.fovy = tlcamera["fov"].toDouble();
    if(tlcamera.contains(QString("nearClip"))) result.near_clip = tlcamera["nearClip"].toDouble();
    if(tlcamera.contains(QString("farClip"))) result.far_clip = tlcamera["farClip"].toDouble();
    if(tlcamera.contains(QString("focalDistance"))) result.mFocalLength = tlcamera["focalDistance"].toDouble();
    if(tlcamera.contains(QString("lensRadius"))) result.mLensRadius = tlcamera["lensRadius"].toDouble();
